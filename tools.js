(function () {
  var T = [
    {
      ...
      exec: function (q) {
        return fetch('https://en.wikipedia.org/api/rest_v1/page/summary/' + encodeURIComponent(q))
          ...
          .then(function (d) {
            return (d.title ? '**' + d.title + '**: ' : '') + (d.extract || 'No summary found.');
          })
          .catch(function () { return 'Could not fetch Wikipedia.'; });
      }
    },
    {
      id: 'country', name: 'Countries', icon: '\u{1F30D}',
      match: function (t) {
        var m = t.match(/(?:population|capital|currency|language|gdp|area|region|info|details?|about)\s+(?:of|for|in)\s+(?:the\s+)?(?:country\s+)?(.+)/i);
      ...
      exec: function (q) {
        return fetch('https://restcountries.com/v3.1/name/' + encodeURIComponent(q))
          .then(function (r) { return r.json(); })
          .then(function (d) {
            if (!d.length || d.message) return 'Country not found.';
            var c = d[0];
            return '**' + (c.name ? c.name.common : q) + '** | Capital: ' + (c.capital ? c.capital[0] : 'N/A') +
              ' | Pop: ' + ((c.population || 0) / 1e6).toFixed(1) + 'M | Region: ' + (c.region || 'N/A') +
    ...
    },
    {
      ...
      match: function (t) {
        var m = t.match(/(?:weather|temperature|forecast|rain|snow|humid|wind|climate)\s+(?:in|at|for|of)\s+(.+)/i);
        return m ? m[1].replace(/[?.!,]+$/, '').trim() : null;
      },
      exec: function (q) {
        return fetch('https://wttr.in/' + encodeURIComponent(q) + '?format=j1')
          ...
          .then(function (d) {
            ...
            return '**' + q + '**: ' + c.temp_C + '\u00B0C (feels ' + c.FeelsLikeC + '\u00B0C), ' +
              (c.weatherDesc ? c.weatherDesc[0].value : '') +
              ' | Humidity: ' + c.humidity + '% | Wind: ' + c.windspeedKmph + ' km/h | Visibility: ' + c.visibility + ' km';
          })
    ...
    },
    {
      ...
      match: function (t) {
        var m = t.match(/(?:pokemon|pok[e\u00E9]dex)\s+(.+)/i);
        return m ? m[1].replace(/[?.!,]+$/, '').trim() : null;
      },
      exec: function (q) {
        return fetch('https://pokeapi.co/api/v2/pokemon/' + encodeURIComponent(q.toLowerCase()))
          ...
          .then(function (d) {
            return '**' + d.name + '** #' + d.id + ' | Type: ' + d.types.map(function (t) { return t.type.name; }).join('/') +
              ' | ' + d.height / 10 + 'm | ' + d.weight / 10 + 'kg | HP:' + d.stats[0].base_stat +
    ...
    },
    {
      ...
      exec: function () {
        return fetch('https://api.adviceslip.com/advice')
          .then(function (r) { return r.json(); })
          .then(function (d) { return d.slip ? d.slip.advice : 'N/A'; })
    ...
    },
    {
      ...
      exec: function (n) {
        return fetch('http://numbersapi.com/' + n + '?json')
          .then(function (r) { return r.json(); })
          .then(function (d) { return '**' + d.number + '**: ' + d.text; })
    ...
    },
    {
      ...
      exec: function () {
        return fetch('https://bored-api.appbrewery.com/random')
          .then(function (r) { return r.json(); })
          .then(function (d) { return '**' + d.activity + '** (' + d.type + ', ' + d.participants + ' person' + (d.participants > 1 ? 's' : '') + ')'; })
    ...
    },
    {
      ...
      exec: function () {
        return fetch('https://ipapi.co/json/')
          .then(function (r) { return r.json(); })
          .then(function (d) {
            return '**IP:** ' + d.ip + ' | **City:** ' + d.city + ', ' + d.region +
              ' | **Country:** ' + d.country_name + ' (' + d.country_code + ')' +
              ' | **ISP:** ' + d.org + ' | **TZ:** ' + d.timezone;
          })
    ...
    },
    {
      ...
      match: function (t) {
        var m = t.match(/(?:bitcoin|btc|ethereum|eth|solana|sol|doge|dogecoin|xrp|cardano|ada|polygon|matic|polkadot|dot|avalanche|avax|chainlink|link|uniswap|uni|shib|shiba|tron|trx|ton|pepe|wif)\b/i);
        if (m) return m[0];
        var m2 = t.match(/(?:crypto|coin|token)\s+(?:price|of)?\s*(\w+)/i);
        return m2 ? m2[1] : null;
      },
      exec: function (q) {
        return fetch('https://api.coingecko.com/api/v3/search?query=' + encodeURIComponent(q))
          .then(function (r) { return r.json(); })
          .then(function (d) {
            if (!d.coins || !d.coins.length) return 'Cryptocurrency not found: ' + q;
            var c = d.coins[0];
            return '**' + c.name + '** (' + c.symbol.toUpperCase() + ')' +
              ' | Rank: #' + c.market_cap_rank +
              (c.price_btc ? ' | ' + c.price_btc.toFixed(8) + ' BTC' : '');
          })
          .catch(function () { return 'Could not fetch crypto data.'; });
      }
    },
    {
      ...
      exec: function (q) {
        return fetch('https://open.er-api.com/v6/latest/USD')
          .then(function (r) { return r.json(); })
          .then(function (d) {
            var r = d.rates, w = q.toUpperCase().split(/\s+/), f = [];
            w.forEach(function (c) { if (r[c]) f.push(c + ': ' + r[c].toFixed(4) + ' USD'); });
            return f.length ? '**Rates (per USD):**\n' + f.join('\n') : 'No matching currencies found in: ' + q;
          })
      ...
      exec: function () {
        return fetch('https://opentdb.com/api.php?amount=1&type=multiple')
          ...
          .then(function (d) {
            ...
            var q = d.results[0], a = [q.correct_answer].concat(q.incorrect_answers).sort(function () { return Math.random() - 0.5; });
    ...
    },
    {
      ...
      match: function (t) {
        var m = t.match(/(?:github|gh)\s+(?:user|profile|repo|stats?)\s+(?:of|for|about)?\s*@?(\w[\w-]*)/i);
        return m ? m[1] : null;
      },
      exec: function (q) {
        return fetch('https://api.github.com/users/' + encodeURIComponent(q))
          .then(function (r) { return r.json(); })
          .then(function (d) {
            if (d.message) return 'GitHub user not found: ' + q;
            return '**' + d.login + '** | ' + (d.name || 'N/A') +
              ' | Bio: ' + (d.bio || 'No bio') +
              ' | Repos: ' + d.public_repos +
              ' | Followers: ' + d.followers +
              ' | Location: ' + (d.location || 'N/A');
          })
          .catch(function () { return 'Could not fetch GitHub profile.'; });
      }
    },
    {
      id: 'meal', name: 'Recipe', icon: '\u{1F35A}',
      match: function (t) {
        var m = t.match(/(?:recipe|meal|food|cook|dish)\s+(?:for|of|with)?\s*(.+)/i);
        return m ? m[1].replace(/[?.!,]+$/, '').trim() : null;
      },
      exec: function (q) {
        return fetch('https://www.themealdb.com/api/json/v1/1/search.php?s=' + encodeURIComponent(q))
          .then(function (r) { return r.json(); })
          .then(function (d) {
            if (!d.meals || !d.meals.length) return 'No recipe found for: ' + q;
            var m = d.meals[0], ings = [];
            for (var i = 1; i <= 20; i++) {
              if (m['strIngredient' + i]) {
                ings.push('- ' + m['strIngredient' + i] + ' (' + (m['strMeasure' + i] || '') + ')');
              }
            }
            return '**' + m.strMeal + '** | ' + m.strCategory + ' | ' + m.strArea +
              '\n\n**Ingredients:**\n' + ings.join('\n') +
              '\n\n**Instructions:**\n' + m.strInstructions;
          })
          .catch(function () { return 'Could not fetch recipe.'; });
      }
    },
    {
      id: 'uni', name: 'Universities', icon: '\u{1F393}',
      match: function (t) {
        var m = t.match(/(?:universit(?:y|ies)|college)\s+(?:in|at|of)\s+(.+)/i);
        return m ? m[1].replace(/[?.!,]+$/, '').trim() : null;
      },
      exec: function (q) {
        return fetch('http://universities.hipolabs.com/search?country=' + encodeURIComponent(q) + '&limit=5')
          .then(function (r) { return r.json(); })
          .then(function (d) {
            if (!d.length) return 'No universities found for: ' + q;
            return d.map(function (u) {
              return '- **' + u.name + '** | ' + (u.country || '') + ', ' + (u['state-province'] || '') +
                (u.web_pages && u.web_pages[0] ? ' | [Site](' + u.web_pages[0] + ')' : '');
            }).join('\n');
          })
    ...
    /* Internal tools (no API calls) */
    {
      ...
      match: function (t) {
        ...
        return m ? { expr: m[1].trim(), extra: m[2] || null } : null;
      },
      exec: function (q) {
        try {
          ...
          if (typeof result === 'number' && !isFinite(result)) return 'Error: result is infinite';
          return '**' + expr.trim() + '** = ' + result;
        } catch (e) {
          return 'Could not calculate: ' + e.message;
        }
      }
    },
    {
    ...
    },
    {
      ...
      match: function (t) {
        var m = t.match(/(?:current|what(?:'s| is) the)?\s*(?:date|time|day|month|year|weekday)/i);
        return m ? new Date().toISOString() : null;
      },
      exec: function (d) {
        try {
          var dt = new Date(d);
  ...
  function fmtCtx(results) {
    ...
    results.forEach(function (r) {
      c += r.icon + ' ' + r.name + ': ' + (r.error ? 'Error \u2014 ' + r.error : r.data) + '\n';
    });
