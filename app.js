...
/* Key — bulletproof */
function gk(){var s=localStorage.getItem('os_userkey');if(s)return s;if(BK!=='__OPENKEY__')return BK;return null}
function hk(){return gk()!==null}
function rku(){if(hk())$dt.classList.add('on');else $dt.classList.remove('on')}

/* Scroll — prevents auto-scroll when user scrolls up during streaming */
...
document.addEventListener('click',function(e){
  ...
  if(e.target.closest('#keyBanner')){var k=prompt('Enter your OpenRouter API key:');if(k&&k.trim()){localStorage.setItem('os_userkey',k.trim());rku();rM();toast('Key saved','ok')}return}
});
...
/* ═══════════════════════════════════════════════════════
 * SEND — Full orchestrated pipeline with planning
 * ═══════════════════════════════════════════════════════ */
async function send(){
  ...
  /* Phase 2: Create and render plan */
  var plan=Planner.createPlan(text,rr.tools,rr);
  Planner.renderPlan(document.getElementById('planArea'),plan);

  /* Phase 3: Execute tools with step tracking */
  var toolCtx='';
  if(rr.tools.length){
    Planner.markStep(1,'active'); // Analyzing
    await tick(300);
    Planner.markStep(1,'done');

    if(rr.tools.length>0)Planner.markStep(2,'active'); // Dispatching
    await tick(200);
    Planner.markStep(2,'done');

    for(var i=0;i<rr.tools.length;i++){
      Planner.markStep(3+i,'active'); // Tool steps
      try{var results=await Agent.execTools(rr.tools);
      var pillsHtml='';
      results.forEach(function(r,j){var pill=document.getElementById('tp'+j);if(pill){pill.className='tool-pill '+(r.error?'err':'done');pill.textContent=r.icon+' '+r.name+(r.error?' \u2717':' \u2713')}pillsHtml+='<span class="tool-pill done">'+results[j].icon+' '+results[j].name+(results[j].error?' \u2717':' \u2713')+'</span>'});
      toolCtx=Agent.toolCtx(results)}catch(e){toolCtx='Tool error: '+e.message}
      Planner.markStep(3+i,'done');
      await tick(100);
    }
  } else {
    Planner.markStep(0,'active');await tick(250);Planner.markStep(0,'done');
  }
  ...
  /* Phase 4: Generate */
  var genIdx=plan.length-2;
  Planner.markStep(genIdx,'active');

  /* Build messages — limit context window for free models */
  ...
  try{
    var res=await fetch(AU,{method:'POST',headers:{'Authorization':'Bearer '+gk(),'Content-Type':'application/json','HTTP-Referer':location.href,'X-Title':'opensky'},body:JSON.stringify({model:MD,messages:apiMsgs,stream:true}),signal:abrt.signal});
    ...
    var revIdx=plan.length-1;
    Planner.markStep(revIdx,'active');
    ...
    Planner.markStep(revIdx,'done');

    /* Remove plan with fade */
    ...
    else{var row3=document.getElementById('aiRow');if(row3)row3.remove();var msg=err.message||'Request failed';if(msg.indexOf('429')!==-1)msg='Rate limited — wait a moment and try again';else if(msg.indexOf('401')!==-1)msg='Invalid API key — click the banner to update it';else if(msg.indexOf('402')!==-1)msg='Payment required — check your OpenRouter credits';toast(msg,'err')}
