...
var convos = JSON.parse(localStorage.getItem('os_c') || '[]'), aId = null, strm = false, abrt = null, ascroll = true, pfiles = [], tTimer = null, tIdx = 0;
...
function gk(){var s=localStorage.getItem('os_k');if(s&&s.length>5)return s;if(BK!=='__OPENKEY__'&&BK.length>5)return BK;return null}
...
/* BG */
(function(){var c=document.getElementById('bgCanvas'),x=c.getContext('2d'),p=[],N=40;
function rz(){c.width=innerWidth;c.height=innerHeight}
function sd(){p=[];for(var i=0;i<N;i++)p.push({x:Math.random()*c.width,y:Math.random()*c.height,r:Math.random()*.7+.1,a:Math.random()*.1+.01,vx:(Math.random()-.5)*.06,vy:(Math.random()-.5)*.04,ph:Math.random()*6.28})}
function dr(t){x.clearRect(0,0,c.width,c.height);for(var i=0;i<p.length;i++){var q=p[i];q.x+=q.vx;q.y+=q.vy;if(q.x<0)q.x=c.width;if(q.x>c.width)q.x=0;if(q.y<0)q.y=c.height;if(q.y>c.height)q.y=0;var f=.5+.5*Math.sin(t*.0004+q.ph);x.beginPath();x.arc(q.x,q.y,Math.max(.1,q.r),0,6.28);x.fillStyle='rgba(255,255,255,'+(q.a*f).toFixed(4)+')';x.fill()}requestAnimationFrame(dr)}
...
if('webkitSpeechRecognition' in window||'SpeechRecognition' in window){var SR=window.SpeechRecognition||window.webkitSpeechRecognition;rec=new SR();rec.continuous=false;rec.interimResults=true;rec.lang='en-US';rec.onresult=function(e){var t='';for(var i=0;i<e.results.length;i++)t+=e.results[i][0].transcript;$i.value=t;ri($i)};rec.onend=function(){mOff()};rec.onerror=function(){mOff()}}
 $mc.addEventListener('click',function(){if(!rec){toast('Not supported','err');return}if(mOn){rec.stop();mOff()}else{rec.start();mOn=true;$mc.classList.add('recording')}});
...
function wH(){return'<div class="welcome"><div class="welcome-logo"><svg style="width:40px;height:40px" viewBox="0 0 32 32" fill="none"><path d="M16 2L28 14L16 26L4 14L16 2Z" stroke="white" stroke-width="1.8"/><path d="M16 8L22 14L16 20L10 14L16 8Z" fill="white"/></svg></div><h2>What can I help with?</h2><p>Free AI by opensky. Tools: Wikipedia, Weather, Countries, Pok\u00E9dex, Dictionary & more. Upload files, voice input, live code preview.</p><div class="suggestions"><button class="sugg" data-s="What is the population of Japan?">Population of Japan?</button><button class="sugg" data-s="Build a todo app with HTML CSS JS">Build a todo app</button><button class="sugg" data-s="Tell me about Pikachu from Pokemon">Pikachu stats</button><button class="sugg" data-s="Who created you and what can you do?">Who are you?</button></div></div>'}
...
/* Generate follow-ups via API */
async function genFU(conv){
  try{var last=conv.slice(-4);var prompt='Based on this conversation, suggest exactly 3 short follow-up questions (under 8 words each). Return ONLY a JSON array of strings. No other text.\n\n'+last.map(function(m){return(m.role==='user'?'User: ':'AI: ')+m.t}).join('\n');
  var r=await fetch(AU,{method:'POST',headers:{'Authorization':'Bearer '+gk(),'Content-Type':'application/json','HTTP-Referer':location.href,'X-Title':'opensky'},body:JSON.stringify({model:MD,messages:[{role:'user',content:prompt}],stream:false})});
  var d=await r.json();var txt=d.choices&&d.choices[0]&&d.choices[0].message&&d.choices[0].message.content||'';
  txt=txt.replace(/```json?/g,'').replace(/```/g,'').trim();
  var arr=JSON.parse(txt);if(Array.isArray(arr))return arr.filter(function(f){return f.length>3&&f.length<60}).slice(0,3)}catch(e){}return[]
}

/* Thinking */
function startThink(el){var steps=Agent.steps();tIdx=0;el.innerHTML='<div class="think-phase" id="tP"><div class="think-dots"><span></span><span></span><span></span></div><span class="think-text" id="tT">'+steps[0]+'</span></div>';
tTimer=setInterval(function(){tIdx=(tIdx+1)%steps.length;var t=document.getElementById('tT');if(t){t.style.opacity='0';setTimeout(function(){if(t){t.textContent=steps[tIdx];t.style.opacity='1'}},150)}},1100)}
function stopThink(){if(tTimer){clearInterval(tTimer);tTimer=null}var el=document.getElementById('tP');if(el){el.style.opacity='0';setTimeout(function(){if(el.parentNode)el.parentNode.removeChild(el)},200)}}
...
async function send(){
  ...
  if(!hk()){toast('Set API key first','err');return}
  if(!aId)nc();var c=ga();
  ...
  /* Phase 1: Route & Execute Tools (Parallelization) */
  var matches=Agent.route(text);
  var toolCtx='';
  if(matches.length){
    var tArea=document.getElementById('toolArea');
    matches.forEach(function(m,i){tArea.innerHTML+='<div class="tool-pill" id="tp'+i+'">'+m.tool.icon+' '+m.tool.name+'...</div>';ss()});
    try{var results=await Agent.execTools(matches);
    results.forEach(function(r,i){var pill=document.getElementById('tp'+i);if(pill){pill.className='tool-pill '+(r.error?'err':'done');pill.textContent=r.icon+' '+r.name+(r.error?' \u2717':' \u2713')}});
    toolCtx=Agent.toolCtx(results)}catch(e){toolCtx=''}
  }
  ...
  /* Build API messages */
  var apiMsgs=[{role:'system',content:Agent.sys()+toolCtx}];
  ...
  try{var res=await fetch(AU,{method:'POST',headers:{'Authorization':'Bearer '+gk(),'Content-Type':'application/json','HTTP-Referer':location.href,'X-Title':'opensky'},body:JSON.stringify({model:MD,messages:apiMsgs,stream:true}),signal:abrt.signal});
  if(!res.ok){var ej=await res.json().catch(function(){return{}});throw new Error((ej.error&&ej.error.message)||('HTTP '+res.status))}

  stopThink();var body=document.getElementById('aiBody');body.innerHTML='';
  ...
  /* Phase 5: Generate follow-ups (API) */
  var fu=await genFU(c.msgs.concat([{role:'assistant',t:full}]));
  c.msgs.push({role:'assistant',t:full,fu:fu});sv();rM()

  /* Phase 6: Memory — extract and store key facts */
  try{var memMatch=full.match(/(?:remember|note|save|store|keep in mind)\s*(?:that|this|the fact)\s*:?\s*(.+)/i);
  if(memMatch){Agent.memory.set(memMatch[1].trim().slice(0,200),'User asked to remember');toast('Memory saved','ok')}}catch(e){}
  ...
  else{var row3=document.getElementById('aiRow');if(row3)row3.remove();toast(err.message||'Failed','err')}
